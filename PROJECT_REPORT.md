...
3. [Research Foundations, Clinical Methodology & Literature Benchmark](#3-research-foundations-clinical-methodology--literature-benchmark)
   ...
   - 3.5 [Evidence-Based Research Alignment & Gap Analysis (Judge-Ready Benchmark)](#35-evidence-based-research-alignment--gap-analysis-judge-ready-benchmark)
   - 3.6 [Comparative Systems & Literature Benchmark Matrix](#36-comparative-systems--literature-benchmark-matrix)
...
### 3.5 Evidence-Based Research Alignment & Gap Analysis (Judge-Ready Benchmark)

The following structured matrix maps the core clinical, systemic, and operational research papers identified in the MediKiosk presentation deck against the specific design choices, architectural differentiators, and judge-facing talking points of MediKiosk:

| Research Study & Citation | What the Research Establishes | Gap / Limitation Identified | What MediKiosk Does Differently | What You Should Say to the Judge |
| :--- | :--- | :--- | :--- | :--- |
| **Verma et al., 2023**<br>([PubMed / Research Study Reference](https://pubmed.ncbi.nlm.nih.gov/?term=Verma+consultation+time+outpatient+history+taking)) | Patient history is an important contributor to diagnosis, while consultation time is severely limited.<br><br>*Deck citations:* **78.58%** correct diagnoses stem from history, **16%** checklist use by practitioners, and an average of **2.4 min/consultation** in high-volume OPDs. | Existing clinical intake workflows depend on an intermediary/health-assistant step and do not make structured history collection a direct, empowering patient interaction. | **Direct Patient-to-AI Interaction** through touch and voice in the patient's native dialect, followed by instant automated structured case generation and HPI synthesis. | *"We took the insight that history quality matters, but instead of adding another intermediary, we move the initial history-taking directly to the patient–AI interaction."* |
| **Rural–Urban TB Study, 2025**<br>([WHO / National TB Elimination Study](https://www.who.int/publications/i/item/9789240084322)) | Highlights a stark **12% rural vs. 2% urban misclassification difference** and strongly associates rural misclassification with missing prior-treatment/history records. | Patient history and previous physical records are fragmented, damaged, or difficult to retrieve, particularly in underserved, rural, and tier-2/3 clinical settings. | **Camera Document Digitization + Longitudinal Patient Timeline**: Snaps physical paper slips and previous prescriptions at the kiosk, parsing medications and prior diagnoses directly into a structured clinical timeline. | *"We don't claim that digitization alone eliminates diagnostic disparity. Our contribution is to close the information-access gap by bringing previous documents and patient history into one structured timeline."* |
| **Anand et al., 2021**<br>([D Y Patil J Health Sci / OpenAlex W4285528664](https://openalex.org/W4285528664) · [DOI: 10.4103/dypj.dypj_4_20](https://doi.org/10.4103/dypj.dypj_4_20) · [Preprint DOI: 10.2196/preprints.27006](https://doi.org/10.2196/preprints.27006)) | Automated history-taking can support documentation and clinical workflow, while inadequate history directly contributes to medication-related problems and adverse events.<br><br>*Paper Title:* **"Automated Interactive Patient History-Taking System"** | Automation alone doesn't solve the problem of standardizing what information should be collected, particularly when different clinical domains require completely different history frameworks. | **Domain-Specific Clinical Pathways**: General OPD uses standard symptom/history flows (SOCRATES / OPQRST), while AYUSH has dedicated questioning covering Prakriti, Vikriti, Agni, Ama, strictly connecting to **SOCRATES + Dashavidha Pariksha**. | *"We are not simply automating questions. We are creating a domain-grounded questioning framework so that automation still follows clinically meaningful structures."* |
| **Sira et al., 2024**<br>([JMIR / Clinician AI Adoption Study](https://www.jmir.org/)) | Identifies **clinician trust, workflow integration, and deployment friction** as the decisive barriers to real-world clinical AI adoption. | A technically capable AI system will still fail if doctors cannot verify, edit, audit, or retain ultimate control over its outputs ("black box" distrust). | **Physician-in-the-Loop Architecture**: Editable structured summaries, full explainability traces ("Why?" evidence buttons showing classical Charaka/Sushruta and symptom rules), and clear override/audit sign-off capability. | *"We treat the physician as the final authority. AI prepares and structures the information; the physician reviews, edits, and makes the clinical decision."* |


### 3.6 Comparative Systems & Literature Benchmark Matrix
